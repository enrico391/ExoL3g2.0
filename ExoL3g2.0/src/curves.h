...
float getCurveAnca( float time,float scale){
    if(scale<=1){
        return scale*(a[0] + a[1]*cos(time*w)+ b[0]*sin(time*w) + a[2]*cos(2*time*w) + b[1]*sin(2*time*w));
    }else {
        return a[0] + a[1]*cos(time*w)+ b[0]*sin(time*w) + a[2]*cos(2*time*w) + b[1]*sin(2*time*w);
    }
}
...
/// @param scale the scale factor for start with a small curve
/// @return angle for control motor
float getCurveGinocchio(float time, float scale){
    if(scale<=1){
        return scale*(a_g[0] + a_g[1]*cos((time)*w_g)+ b_g[0]*sin((time)*w_g) + a_g[2]*cos(2*(time)*w_g) + b_g[1]*sin(2*(time)*w_g));
    }else{
        return (a_g[0] + a_g[1]*cos((time)*w_g)+ b_g[0]*sin((time)*w_g) + a_g[2]*cos(2*(time)*w_g) + b_g[1]*sin(2*(time)*w_g));
    }}
